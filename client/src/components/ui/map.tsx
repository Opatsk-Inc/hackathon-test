...
type MapRouteProps = {
  ...
  interactive?: boolean
  /** Enable pulse animation for highlighted routes (default: false) */
  animated?: boolean
  /** Animation speed in ms per cycle (default: 1500) */
...
function MapRoute({
  ...
  useEffect(() => {
    ...
    const baseOpacity = Math.max(0, Math.min(1, opacity))
    const baseWidth = Math.max(0.5, width)
    // Keep pulse inside valid [0..1] range for MapLibre line-opacity.
    const amplitude = Math.min(0.15, baseOpacity, 1 - baseOpacity)
    const widthAmplitude = 1 // width swing in pixels

    const animate = (timestamp: number) => {
      if (!map || !map.getLayer(layerId)) return
      const t = (timestamp % animationSpeed) / animationSpeed
      const pulse = Math.sin(t * Math.PI * 2)
      const currentOpacity = Math.max(
        0,
        Math.min(1, baseOpacity + amplitude * pulse)
      )
      const currentWidth = Math.max(0.5, baseWidth + widthAmplitude * pulse)
      map.setPaintProperty(layerId, "line-opacity", currentOpacity)
      map.setPaintProperty(layerId, "line-width", currentWidth)
  ...
  }, [animated, isLoaded, map, layerId, opacity, width, animationSpeed])
